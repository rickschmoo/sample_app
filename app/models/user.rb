...
class User < ActiveRecord::Base
  ...
  validates :password, :presence => true,
            :confirmation => true, #  automatially create the virtual attribute 'password_confirmation'
            :length => {:within => 6..40}
  ...
  def self.authenticate(email, submitted_password)
    ...
    return nil if user.nil? # no such email, i.e. invalid user
